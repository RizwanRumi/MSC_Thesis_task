...
destination_video_dir = "./yolov7_videos_with_inference/"
...
class Detection:
    ...
    def write_fps(self, file):
        file = destination_video_dir +'v7_write_fps.txt'
...
if __name__ == "__main__":
    # Load network
    model_name = './models/iteration_9_v7_best.onnx'
    label_file = 'classes.txt'
    fps_file = destination_video_dir + 'v7_write_fps.txt'

    model = ModelConfiguration(model_name)
    net = model.modelConfig("YOLOV7")
    ...
    if net is not None:
        ...
        if capture.isOpened():
            ...
            result = cv.VideoWriter(destination_video_dir + 'yolov7_output_cpu_core_i5.avi', cv.VideoWriter_fourcc(*'XVID'), 20.0, size)
